//
// Created by notnaturalselection on 08.10.2021.
//
...
//void print_arr(const uint32_t *numbers, const int32_t size) {
...
//uint32_t *read_from_csv(char*filename) {
...
#pragma omp parallel default(none) shared(cycles, arr_to_sort, src_arr, elements, arr_sorted, err_count, max, min, stderr, time_sum, real_time_sum, sort, arr_split) if(omp_get_max_threads() > 1)
    {
#pragma omp single
        {
